...
public class Tokenizer {
	...
	/**
	 * Getter for the token in TOKENS at the position represented by the COUNTER index;
	 *
	...
	/**
	 * Setter for the current indec of the token stream; Retreat the current
	 * token;
	 */
	...
	/**
	 * Reset COUNTER to 0; Only required for printing the tokens in the partial submission;
	 */
			...
			while (i < len) {
				...
				if (Character.isDigit(first)) {
					...
					while (j < len && !WHITESPACE.contains(String.valueOf(line.charAt(j)))
							...
							&&
							!SPECIAL.contains(String.valueOf(line.charAt(j))))
							j++;
					TOKENS.add(line.substring(i, j));
				}
